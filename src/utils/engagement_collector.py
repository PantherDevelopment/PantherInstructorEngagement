...
class EngagementCollector:
    ...
    def collect_instructor_data(
        ...
        """
        cancel_check: optional callable returning True if the report has
        been cancelled. Checked between batches so a worker can stop early
        instead of running all 6 batches to completion after cancellation.
        """
        data = {
            'course_id':    course_id,
        ...
        if cancelled():
            errors.append('Cancelled before completing all data collection')
            data['collection_notes'] = '; '.join(errors)
            return data
        ...
        # Batch 3 - discussions (checks cancellation between topics internally too)
        merge(self.batch3_discussions(course_id, instructor_id, period_start, period_end, period, cancel_check))
        if cancelled():
            errors.append('Cancelled before completing all data collection')
            data['collection_notes'] = '; '.join(errors)
            return data
    ...
    # -----------------------------------------------------------------------
    # BATCH 3: Discussions
    # Uses the /view endpoint to fetch a topic's entire entry+reply tree
    # in a single call, instead of one call per entry plus one per reply.
    # Only instructor REPLIES are tracked (top-level posts are often
    # auto-generated by course shells and aren't meaningful signal).
    # current_week: week replies + term total
    # entire_term:  term total only
    ...
    def batch3_discussions(
        ...
        try:
            ...
            for topic in topics:
                # Cooperative cancellation check - between topics, which is
                # now cheap since /view fetches a whole topic in one call
                if cancel_check and cancel_check():
                    ...
                    result['_error'] = 'Cancelled - discussion counts incomplete'
                    return result

                topic_id = topic.get('id')
                try:
                    view_data = self.api._make_request(
                        'GET',
                        f'/api/v1/courses/{course_id}/discussion_topics/{topic_id}/view'
                    )
                    view_entries = view_data.get('view', []) or []

                    for reply in self._flatten_view_replies(view_entries):
                    ...
                    # Record failed topic so reply counts flagged as incomplete
                    failed_topics += 1
                    continue
        ...
        if error_msg:
            ...
            result['_error'] = (
                f'Discussion counts incomplete - {failed_topics} topic(s) failed to load'
            )

        return result

    def _flatten_view_replies(self, entries, _depth=0):
        """
        Recursively collect every reply (at any nesting depth) from
        a discussion topic's /view response. Each entry may have a
        'replies' list, and each reply may itself have nested replies.
        """
        all_replies = []
        for entry in entries:
            ...
            for reply in replies:
                ...
                all_replies.extend(self._flatten_view_replies([reply], _depth + 1))
        return all_replies
    ...
    def batch4_submissions(self, course_id: str) -> Dict:
        ...
        try:
            ...
            for assignment in assignments:
                ...
                try:
                    ...
                    # Do NOT silently skip - record which assignment failed
                    # so grading counts are flagged as potentially incomplete
                    failed_assignments.append(a_name or str(a_id))
                    continue
        ...
        if failed_assignments:
            result['_error'] = (
                f'Grading counts incomplete - failed to check '
                f'{len(failed_assignments)} assignment(s): '
                + ', '.join(failed_assignments[:3])
                + ('...' if len(failed_assignments) > 3 else '')
            )
        return result
